...
class MatchMatrix {
    ...
    static bool match(char a, char b) {
        // cout << "(a, b) = " << a << ' ' << b << endl;
        // cout << "IUPAC_TO_VALUE(a, b) = " << IUPAC_to_value[a] << ' ' << IUPAC_to_value[b] << endl;
        // bool res = get_value_from_matrix(match_matrix, IUPAC_map_count, IUPAC_to_value[a], IUPAC_to_value[b]);
        // cout << "result of match is " << res << endl;
        // char b = -1 (complement['$'])
        // IUPAC_to_value[b]
        // IUPAC_to_value[-1] = 0
...
void addPalindromes(set<tuple<int, int, int>>* palindromes, unsigned char* S, int S_n, int n, INT * invSA, INT * LCP, rmq_succinct_sct<> rmq, tuple<int, int, int, int> params) {
    ...
    for (double c = 0; c <= (n - 1); c += 0.5 ) {
        ...
        // DEBUG
        if (false) {
        cout << "Parameters: isOdd=" << isOdd << " c=" << c << endl;
        cout << "Calling RLCM with i=" << i << " j=" << j << " n=s_n=" << S_n << endl;
        }
        ...
        if ( !valid_start_locs.empty() and !valid_end_locs.empty() ) {
            ...
            while( start_it != valid_start_locs.end() and end_it != valid_end_locs.end()) {
                ...
                // for (auto& p : valid_end_locs) {
                //     int f = p.first;
                //     int s = p.second;
                //     cout << f << ' ' << s << endl;
                // }
                while (mismatch_diff <= mismatches and end_it != valid_end_locs.end()) {
                    // cout << "before: " << end_it->first << ' ' << end_it->second << endl;
                    end_it = next(end_it);
                    // cout << "after: " << end_it->first << ' ' << end_it->second << endl;
...
;
    ...
    cout << "CLI took " << duration.count() << " seconds." << endl;
    cout << "TODO TIME THE REST (ALSO MAKE THIS A FLAG)" << endl;
    // return 0;
    ...
    // for (auto& pair : IUPAC_map) {
    //     char fst = pair.first;
    //     set<char> snd = pair.second;
    //     cout << fst << ' ';
    //     for (auto& x : snd) {
    //         cout << x << ' ';
    //     }
    ...
    // char value_to_IUPAC[IUPAC_map_count]; // UNUSED
    ...
    // Build complement array (remember that complement is an int array)
    complement['a'] = 't';
    complement['c'] = 'g';
    complement['g'] = 'c';
    complement['t'] = 'a';
    ...
    // cout << IUPAC_to_value[-1] << endl;
    // for (auto& x : IUPAC_to_value) {
    //     cout << x << ' ';
    // }
    // cout << '\n' << IUPAC_to_value[-1] << endl;
    // for (int i = 0; i < IUPAC_map_count; ++i) {
    //     for (int j = 0; j < IUPAC_map_count; ++j) {
    //         bool res = MatchMatrix::match(letters[i], complement[letters[j]]);
    //     }
    // }
    ...
    cout << "Found " << palindromes.size() - 1 << " palindromes" << endl;
